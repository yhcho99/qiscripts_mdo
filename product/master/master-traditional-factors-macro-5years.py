...
IDENTIFIER = "master_test_traditional_factors_macro_5years_v2"
CACHE_PATH = f"cache_{IDENTIFIER}"
DATASET_PATH = f"dataset_{IDENTIFIER}"
...
if __name__ == "__main__":
    ...
    with mp.Pool(8) as pool:
        raw_inputs1 = pool.starmap(load_dataset.call_if_not_loaded, input_data1)
    ...
    with mp.Pool(8) as pool:
        raw_inputs2 = pool.starmap(load_dataset.call_if_not_loaded, input_data2)
    ...
    infer_filter_base = infer_filter_base
    ...
    basket2_filter = infer_filter_base.copy()
    basket2_filter.loc[:, unselected_keys] = False
    
    # NYSE and NASDAQ Top 500 Filter
    top500_filter = load_dataset.call_if_not_loaded("mv", ["high_level", "equity", "get_monthly_market_value"])
    top500_filter._data = top500_filter._data[infer_filter_base._data]
    top500_filter = top500_filter.rank(ascending=False, pct=False) <= 500

    # NASDAQ Top 100 Filter
    nasdaq_config = copy.deepcopy(setting_params["kirin_config"])
    nasdaq_config["cache_dir"] = Path(nasdaq_config["cache_dir"]).parent / (CACHE_PATH + "_nasdaq_only")
    nasdaq_config["exchange"] = ["NASDAQ"]
    nasdaq_universe = Universe(**nasdaq_config)
    nasdaq_load_dataset = LoadData(path=Path(DATA_DIR) / (DATASET_PATH + "_nasdaq_only"), universe=nasdaq_universe)
    top100_filter = nasdaq_load_dataset.call_if_not_loaded("mv", ["high_level", "equity", "get_monthly_market_value"])
    top100_filter._data = top100_filter._data.reindex(infer_columns, axis=1)
    top100_filter._data = top100_filter._data[infer_filter_base._data]
    top100_filter = top100_filter.rank(ascending=False, pct=False) <= 100
    
    infer_filter_list = [basket2_filter, top500_filter, top100_filter]
    name_list = ["BASKET2", "TOP500" , "TOP100"]
