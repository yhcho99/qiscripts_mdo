...
setting_params = {
    'identifier': 'hdiv_quant',  # 실험의 이름입니다
    'kirin_config': {
...
_input_length = 1
input_data = [
    ("dy", ['high_level', 'equity', 'get_dividend_yield'], [], {}),
    ("gpa", ['high_level', 'equity', 'get_gpa'], [], {}),
    ("gpm", ['high_level', 'equity', 'get_gp_to_market'], [], {}),
    ("mom_1m_0m", ['high_level', 'equity', 'get_mom'], ['pi', 1, 0]),
    ("hdiv_factor", ['high_level', 'equity', 'get_hdiv_factor'])
]
...
filter_mv = additional_mv.rank(ascending=False, pct=True) <= 0.19

additional_mv._data = additional_mv._data.loc[date_dict['date_from']:date_dict['date_to']]
output = load_dataset.call_if_not_loaded('pr_1m_0m', ['high_level', 'equity', 'get_monthly_price_return'], [1, 0]).masked_by(filter_mv)
output._data = output._data.loc[date_dict['date_from']:date_dict['date_to']]
