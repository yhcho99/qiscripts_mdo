...
from utils.mdo_func import Mdo
...
#################################### 공통설정부분 ##########################################
setting_params = {
    "identifier": "qrft_2022-02-28",  # 실험의 이름입니다
    "kirin_config": {
...
mdo_data = [
    ("pr_1m_0m", ["mdo_func", "get_monthly_price_return"], [1]),
    ("mv", ["mdo_func", "get_monthly_market_value"]),
    ("btm", ["mdo_func", "get_book_to_market"]),
    ("mom_12m_1m", ["mdo_func", "get_price_momentum"], [12, 1]),
    ("ram_12m_0m", ["mdo_func", "get_ram"], [12]),
    ("vol_3m", ["mdo_func", "get_monthly_volatility"], [3]),

    ("at", ["mdo_func", "get_asset_turnover"]),
    ("gpa", ["mdo_func", "get_gpa"]),
    ("rev_surp", ["mdo_func", "get_revenue_surprise"]),
    ("cash_at", ["mdo_func", "get_cash_to_asset"]),
    ("op_lev", ["mdo_func", "get_operating_leverage"]),
    ("roe", ["mdo_func", "get_roe"]),
    ("std_u_e", ["mdo_func", "get_standardized_unexpected_earnings"]),
    ("ret_noa", ["mdo_func", "get_return_on_net_operating_asset"]),
    ("etm", ["mdo_func", "get_earnings_to_market"]),
    (
        "ia_mv",
        ["mdo_func", "get_linear_assumed_intangible_asset_to_market_value"],
    ),
    ("ae_m", ["mdo_func", "get_advertising_expense_to_market"]),
    (
        "ia_ta",
        ["mdo_func", "get_linear_assumed_intangible_asset_to_total_asset"],
    ),
    ("rc_a", ["mdo_func", "get_rnd_capital_to_asset"]),
    ("r_s", ["mdo_func", "get_rnd_to_sale"]),
    ("r_a", ["mdo_func", "get_rnd_to_asset"])
]
#################################### integration ##########################################
...
mdo_params = [
    ...
    "CS_PPENT_A",
    "CS_ACT_A",
    "CS_LCT_A",  # net operating assets = CS_PPENT + CS_ACT - CS_LCT
    "CS_IB_A",
    "CS_IBQ_Q",
    "CS_MKVALT_A",
    "CS_MKVALTQ_Q",
...
class MdoUniverse:
    def __init__(self) -> None:
        self.days = mdo.DateRange(startDate="2000-1-1", endDate="2022-8-1", periodType='month', dom=31)
        self.universe = mdo.DimensionUniverse(items="DS_SECURITY")
        self._config_universe()

    def _config_universe(self):
        items = ["DS_SECTYPE", "DS_IS_MAJOR"]
        mdo.PlugData(universe=self.universe, items=items, update=True)
        self.universe.data = self.universe.data[
            (self.universe.data["DS_IS_MAJOR"] == "Y")
            & (self.universe.data["DS_SECTYPE"] == "Ordinary Share")
        ]

        self.universe = mdo.CustomUniverse(day=self.days, security=list(self.universe.data["security"]))
        mdo.PlugData(universe=self.universe, items=["DS_EXCHANGE", "DS_CLOSE"], update=True)

        self.universe.data = self.universe.data[
            self.universe.data["DS_EXCHANGE"].isin(
                [
                    "NEW YORK SE(COMPOSITE)",
                    "NYSE ARCA(COMPOSITE)",
                    "NYSE American",
                    "NASDAQ(COMPOSITE)",
                ]
        ...
        mdo.FundamentalData(universe=self.universe, items=mdo_params, update=True)
        self.universe.data = self.universe.data.set_index('day')
...
if __name__ == "__main__":
    ...
    mdo_universe = MdoUniverse().universe

    load_dataset = LoadData(path=Path(DATA_DIR) / "kirin_dataset", universe=universe)
    load_mdo = Mdo(universe=mdo_universe)

    with mp.Pool(4) as pool:
        res_mdo = pool.starmap(load_mdo.call_if_not_loaded, mdo_data)
    ...
    filter_mv = load_dataset.call_if_not_loaded(
        "mv", ["high_level", "equity", "get_monthly_market_value"]
    )
    filter_mv = filter_mv.rank(ascending=False, pct=True) <= 0.15
    ...
    st = Strategy(
        ...
        long_picking_config=picks.picking_by_signal(
            "out", False, 1, None, ascending=False
        ),
        long_weighting_config=(
            weights.rank_sum_discounted_mix_with_top_market_weight(0.995, 70, 350, 0.5),
