...
test_harvest_config = {
        "emissions_fp" : "./harvest/emissions_intensity_per_sales.xlsx",
        }
...
class Harvest:
    ...
    '''
    def gvkey_iid_to_ticker(gvkey_iid):
        ticker = mm.loc[gvkey_iid]
        if isinstance(ticker, pd.DataFrame):
            return ticker.iloc[ticker['effdate'].argmax()]['tic']
        return ticker['tic']
    '''
...
#################################### 공통설정부분 ##########################################
setting_params = {
    "identifier": "Harvest_Green",  # 실험의 이름입니다
    "kirin_config": {
