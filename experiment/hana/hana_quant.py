...
setting_params = {
    "identifier": f"BTS_model_{MODEL}_NVQ{int(NVQ_CF * 100):2d}_AMOM{int(AMOM_CF * 100):2d}",  # 실험의 이름입니다
    'kirin_config': {
...
input_data = [
    ...
    ("mom_12m_1m", ["high_level", "equity", "get_mom"], ["pi", 12, 1]),
]
...
sector = api.compustat.get_historical_gics("sector").fillna(-1).astype(int).astype(str)
sector_mask = (sector != "40") & (sector != "-1")
ndx_mask = MV[sector_mask].rank(axis=1, ascending=False) <= 150
filter_nxd =  QraftData(name="filter_nxd", data=ndx_mask)
...
for i, qdata in enumerate(res):
    ...
    qdata = qdata.masked_by(ndx_mask)
...
input_x = bm.DataBinder(
    ...
    training_filter=filter_nxd,
    inference_filter=filter_nxd,
