...
from utils.hana_universe_gvkey import HANA_UNIVERSE
...
#################################### 공통설정부분 ##########################################
setting_params = {
    "identifier": f"BTS_model_{MODEL}_NVQ{int(NVQ_CF*100):2d}_AMOM{int(AMOM_CF*100):2d}_MV{MV_MASK}_FILTER{MASK}",  # 실험의 이름입니다
    "kirin_config": {
        "cache_dir": (Path(DATA_DIR) / "hana_meta_cache").as_posix(),
...
if __name__ == "__main__":
    ...
    MV = load_dataset.call_if_not_loaded("mv", ["high_level", "equity", "get_monthly_market_value"])

    sector = api.compustat.get_historical_gics(gics_level='sector')
    sector_mask = sector != 40

    # inference filter
    meta_mask = filter_training.copy()
    infer_universe = HANA_UNIVERSE
    difference = meta_mask._data.columns.difference(infer_universe)
    meta_mask.loc[:, difference] = False


    ndx_mask = MV[sector_mask].rank(axis=1, ascending=False) <= MV_MASK
    ndx_mask = QraftData("ndx_mask", ndx_mask)

    #NVQ 필터
    index_book_to_market = load_dataset.call_if_not_loaded("index_btm", ["high_level", "equity", "get_index_for_book_to_market"]).masked_by(ndx_mask)
    index_linear_intangible_to_tangible = load_dataset.call_if_not_loaded("index_ia_ta", ["high_level", "equity", "index_for_linear_assumed_intangible_asset_to_total_asset"]).masked_by(ndx_mask)

    norm_index_book_to_market = index_book_to_market.zscore()
    norm_index_linear_intangible_to_tangible = index_linear_intangible_to_tangible.zscore()

    nvq_filter = 0.3 * norm_index_book_to_market + 0.7 * norm_index_linear_intangible_to_tangible

    #AMOM필터
    mom_12m_1m = load_dataset.call_if_not_loaded("mom_12m_1m", ["high_level", "equity", "get_mom"], ["pi", 12, 1]).masked_by(ndx_mask)
    gpa = load_dataset.call_if_not_loaded("gpa_trailing", ["high_level", "equity", "get_gpa"], [], {'as_trailing':True}).masked_by(ndx_mask)

    norm_gpa = gpa.zscore()
    norm_mom_12m_1m = mom_12m_1m.zscore()

    amom_filter = 0.5 * mom_12m_1m + 0.5 * norm_mom_12m_1m

    #BTS필터
    bts_filter = NVQ_CF * nvq_filter + AMOM_CF * amom_filter
    bts_filter = (bts_filter).rank(ascending=False) <= MASK
    filter_infer = bts_filter
