...
#################################### 공통설정부분 ##########################################
setting_params = {
    "identifier": "qrft0512_test_filter_vol",  # 실험의 이름입니다
    "kirin_config": {
...
strategy_dict = {
    ...
    "long_picking_config": picks.picking_by_signal("out", False, 1, None, ascending=False),  # 롱포트폴리오 뽑는방법
...
if __name__ == "__main__":
    ...
    gpa = load_dataset.call_if_not_loaded("gpa", ["compustat", "custom_api", "get_gpa"])
    etm = load_dataset.call_if_not_loaded("etm", ["high_level", "equity", "get_earnings_to_market"])
    vol = load_dataset.call_if_not_loaded("vol_3m", ["high_level", "equity", "get_monthly_volatility"], [3])
    mom = load_dataset.call_if_not_loaded("mom_12m_1m", ["high_level", "equity", "get_monthly_price_return"], [12, 1])
    ...
    gpa_score = gpa.masked_by(filter_mv).rank(ascending=True, pct=True)
    etm_score = etm.masked_by(filter_mv).rank(ascending=True, pct=True)
    vol_score = vol.masked_by(filter_mv).rank(ascending=False, pct=True)
    mom_score = mom.masked_by(filter_mv).rank(ascending=True, pct=True)
    filter_inference = (vol_score).rank(ascending=False, pct=False) <= 500
