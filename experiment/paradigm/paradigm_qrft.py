...
#################################### 공통설정부분 ##########################################
setting_params = {
    "identifier": "paradigm_test_quarterly_2",  # 실험의 이름입니다
    "kirin_config": {
...
if __name__ == "__main__":
    ...
    filter_mv = filter_mv.rank(ascending=False, pct=True) <= 1.0
    ...
    paradigm_filter = pd.read_csv("/home/sronly/Projects/qiscripts/experiment/paradigm/PARADIGM_inferfilter.csv",index_col=0,parse_dates=True,dtype='str')
    paradigm_filter = paradigm_filter.loc["2010-12-31":,:]

    paradigm_filter = paradigm_filter.astype(bool)
    paradigm_filter = QraftData("inference_filter",paradigm_filter)
    input_x = bm.DataBinder(
       data_list=input_data,
       training_filter=filter_mv,
       inference_filter=paradigm_filter,
