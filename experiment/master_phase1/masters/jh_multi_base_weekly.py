...
from qraft_data.util import get_kirin_api, get_compustat_api
...
def qrft_running(_, context_name_list, lc_list, shapes_dict, d_inference):
    ...
    ''' 
    Model 실행하기 위한 준비과정.
    1. _proceed 모듈에서 saver, summary 초기화
    2. EarlyStopping 인스턴스 생성
    3. Epoch 0인 단계에서의 weights 저장
    4. 학습을 진행하면서 이전 배치에서 사용된 weights 공유 여부 판단.
    '''
...
class LoadDataWeekly:
    ...
    def call_if_not_loaded_weekly(self, name: str, method_names: list, args=tuple(), kwargs=None) -> QraftData:
        while True:
            if kwargs is None:
                kwargs = {}
            try:
                ...
                try:
                    if method_names == 'trading':
                        ...
                        obj = get_compustat_api(self.universe)
...
if __name__ == "__main__":
    ...
    compustat_api = get_compustat_api(universe)

    load_dataset = LoadData(path=Path(DATA_DIR) / "jh_master_dataset", universe=universe)
    load_dataset_weekly = LoadDataWeekly(path=Path(DATA_DIR) / "jh_master_dataset", universe=universe)

    with mp.Pool(8) as pool:
       res = pool.starmap(load_dataset.call_if_not_loaded, input_data)

    # TODO Input 에 일별 데이터 만들고, 합치기

    additional_mv = load_dataset_weekly.call_if_not_loaded_weekly("mv_weekly", ["get_daily_market_value"])
    additional_mv._data = additional_mv._data.resample('W-FRI').ffill()
    additional_mv = additional_mv.minmax()

    training_filter_mv = load_dataset_weekly.call_if_not_loaded_weekly("mv_weekly", ["get_daily_market_value"])
    training_filter_mv._data = training_filter_mv._data.resample('W-FRI').ffill()
    training_filter_mv._data = training_filter_mv._data.notna()

    infer_filter_base = load_dataset_weekly.call_if_not_loaded_weekly("mv_weekly", ["get_daily_market_value"])
    infer_filter_base._data = infer_filter_base._data.resample('W-FRI').ffill()
    infer_filter_base._data = infer_filter_base._data.notna()

    output = load_dataset_weekly.call_if_not_loaded_weekly("pr_1m_0m_weekly", ["get_daily_price_data"])
    output._data = output._data.resample('W-FRI').ffill()
    output = output.winsorize((0.01, 0.99), pct=True).zscore()

    weekly_index = output._data.index
    weekly_columns =  output._data.columns
    ...
    for i, qdata in enumerate(res):
       if qdata.get_tag() == QraftDataTag.INDEX.value:
           ...
           qdata._data = qdata._data.reindex(weekly_index,method='ffill')

       if qdata.get_tag() == QraftDataTag.EQUITY.value:
           if qdata.name == "sector":
               qdata = qdata.one_hot(sector_values)
               qdata._data = qdata._data.reindex(weekly_index, method='ffill')[weekly_columns]
           else:
               qdata = qdata.rank(ascending=True, pct=True)
               qdata.data = qdata._data.reindex(weekly_index, method='ffill')[weekly_columns]
