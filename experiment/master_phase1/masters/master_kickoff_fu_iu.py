...
setting_params = {
    "identifier": "active_ls/short",  # 실험의 이름입니다
    "kirin_config": {
...
if __name__ == "__main__":
    ...
    for d_inference in dates_inference:
        with sbm.local_context(
            inference_dates=d_inference, length=0
        ) as lc:
            for meta, data in lc.inference_iterate():
                sec_keys = meta["SECURITY"]
                infer = pd.DataFrame([[1] * len(sec_keys)], index=["out"], columns=sec_keys)
                filepath = Path(DATA_DIR) / setting_params["identifier"] / "infer" / f"infer_at_{d_inference.strftime('%Y-%m-%d')}.csv"
                filepath.parent.mkdir(parents=True, exist_ok=True)
                infer.to_csv(filepath)
