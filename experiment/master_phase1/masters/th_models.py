...
def tensor_nanmean(x: torch.Tensor, dim, keepdim):
    ...
    return torch.where(num_finite != 0, sum_finite / num_finite, torch.zeros_like(sum_finite))
...
class Attention(nn.Module):
    def __init__(self, in_features, out_features, dropout=0., layer_norm=False, kind="sample"):
        nn.Module.__init__(self)
...
class SampleImputation(nn.Module):
    ...
    def forward(self, x, add_noise=True):
        ...
        finite_num = torch.where(finite_num > 0.5, finite_num, torch.ones_like(finite_num))
        weighted = (x * self.weight).sum(dim=3, keepdim=False) / finite_num  # [B, U, T, F]
...
class Net(nn.Module):
    def __init__(
        ...
        nn.Module.__init__(self)
        assert cross_units[-1] == batch_units[0]
        ...
        for in_features, out_features in zip(cross_units[:-1], cross_units[1:]):
            cross_layers.append(Attention(in_features, out_features, dropout, layer_norm, "sample"))
        self.cross_layers = nn.Sequential(*cross_layers)

        batch_layers = []
        num_layers = len(batch_units) - 1
        for i, (in_features, out_features) in enumerate(zip(batch_units[:-1], batch_units[1:])):
            if i == num_layers - 1:
                batch_layers.append(Attention(in_features, out_features, dropout, False, "batch"))
            else:
                batch_layers.append(Attention(in_features, out_features, dropout, layer_norm, "batch"))
        self.batch_layers = nn.Sequential(*batch_layers)
    
    def _forward(self, x, add_noise):
    ...
    def forward(self, x):
        aggregated, risks, loss_imp, _ = self._forward(x, add_noise=True)
        _, _, _, loss_cov = self._forward(x, add_noise=False)
...
class TestModel(BaseModel):
    def __init__(
        ...
        batch_units = [cross_units[-1]] + [e for e in batch_units]
        ...
        self.optimizer = optim.Adam(list(self.net.parameters()), learning_rate, weight_decay=weight_decay)
        self.epoch = 0
        assert len(stage) == 2
    ...
    def asymmetric_huber_loss(self, predict, y):
        ...
        asymmetric_huber_loss = (
            ...
            + (self.huber_loss_positive_beta * positive_outbound_error - self.huber_loss_positive_beta ** 2 / 2).sum()
            + torch.square(negative_inbound_error).sum() / 2
            + (- self.huber_loss_negative_beta * negative_outbound_error - self.huber_loss_negative_beta ** 2 / 2).sum()
        ) / denom
...
class Net2(nn.Module):
    def __init__(
        ...
        self.imputation_layer = SampleImputation((time_length, cross_units[0]), noise=0.01)
        ...
        for in_features, out_features in zip(serial_units[:-1], serial_units[1:]):
            serial_layers.append(Attention(in_features, out_features, dropout, layer_norm, "sample"))
    ...
    def calculate_cov_loss(x_centered):
        ...
        num_corr = num_features * (num_features - 1) / 2
        adjust = num_features * num_features / num_corr
        z_centered = (x_centered - x_centered.mean(dim=1, keepdim=True)) / x_centered.std(dim=1, keepdim=True)
        corr_matrix = torch.matmul(torch.transpose(z_centered, 1, 2), z_centered) / num_universe
...
class TestModel2(BaseModel):
    def __init__(
        ...
        self.optimizer = optim.Adam(list(self.net.parameters()) + [self.loss_cf], learning_rate, weight_decay=weight_decay)
    ...
    def calculate_step_info_with_loss(self, data: dict, is_infer=False) -> dict:
        ...
        loss, loss_predict, loss_imp, loss_cov, loss_cf = self.calculate_loss(data, is_infer)
        step_info = {
            "loss": loss,
            "loss_predict": loss_predict,
            "loss_imp": loss_imp,
