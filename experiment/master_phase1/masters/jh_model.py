...
class Net(nn.Module):
    def __init__(
        self,
        time_length, cross_units,
        alt_time_length, alt_cross_units,
        ff_time_length, ff_cross_units,
        batch_units, dropout, layer_norm, noise,
    ):
        nn.Module.__init__(self)
        assert cross_units[-1] + alt_cross_units[-1] + ff_cross_units[-1] == batch_units[0]

        self._time_length = time_length
        self._alt_time_length = alt_time_length
        self._ff_time_length = ff_time_length

        self._cross_units = cross_units
        self._alt_cross_units = alt_cross_units
...
#######
        self.alts_imputation_layer = SampleImputation((alt_time_length, alt_cross_units[0]), noise)
        alts_cross_layers = []
        alt_cross_units[0] = 2 * alt_cross_units[0]
        for in_features, out_features in zip(alt_cross_units[:-1], alt_cross_units[1:]):
            alts_cross_layers.append(
                Attention(in_features, out_features, dropout, layer_norm, "sample")
            )
        self.alts_cross_layers = nn.Sequential(*alts_cross_layers)
...
#######
    ...
    def _forward(self, x, alts, ffs, add_noise):
        # x: [32,256,36,67],
        # alts :
        # ffs: [32,256,36,36]
        # origin layers
        ...
        alts, alts_loss_imputation = self.alts_imputation_layer(alts, add_noise)
        alts_cross = tensor_z(alts, dim=-1)
        alts_serial = tensor_z(alts, dim=-2)
        alts = torch.cat([alts_cross, alts_serial], dim=-1)
        alts = self.alts_cross_layers(alts)
    ...
    def forward(self, x, alts, ffs):
        aggregated, risks, loss_imp, _ = self._forward(x, alts, ffs, add_noise=True)
        _, _, _, loss_cov = self._forward(x, alts, ffs, add_noise=False)
...
class TestModel(BaseModel):
    def __init__(
        ...
        cross_units = [num_cross_features] + [e for e in cross_units] # [67, 64, 32]
#######
        alt_time_length = self.data_shape["alts"][0]
        num_alt_cross_features = self.data_shape["alts"][1]
        alt_cross_units = [num_alt_cross_features] + [e for e in alt_cross_units] # [9, 9, 4]
#######
        ff_time_length = self.data_shape["ffs"][0]
        num_ff_cross_features = self.data_shape["ffs"][1]
        ff_cross_units = [num_ff_cross_features] + [e for e in ff_cross_units] # [36, 8, 4]

        batch_units = [cross_units[-1] + alt_cross_units[-1] + ff_cross_units[-1]] + [e for e in batch_units] # [40, 40, 40, 16]
    ...
    def calculate_loss(self, data: dict, is_infer=False):
        ...
        alts = alts.to(self.device).float()
        ...
        aggregated, risks, loss_imp, loss_cov = self.net(x, alts, ffs)
    ...
    def predicts(
        ...
        alts = alts.to(self.device).float()
        ffs = ffs.to(self.device).float()
        _, risks, _, _ = self.net(x, alts, ffs)
