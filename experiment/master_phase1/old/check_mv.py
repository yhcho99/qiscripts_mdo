...
#################################### 공통설정부분 ##########################################
setting_params = {
    "identifier": "master_qrft",  # 실험의 이름입니다
    "kirin_config": {
...
if __name__ == "__main__":
    ...
    training_filter_mv = training_filter_mv.rank(ascending=False, pct=True) <= 0.3


    #  inferenc filter
    infer_filter = load_dataset.call_if_not_loaded("mv", ["high_level", "equity", "get_monthly_market_value"])
    master_gvkey_path = DATA_DIR + '/master_gvkey.csv'
    master_gvkey = pd.read_csv(master_gvkey_path, index_col=0)
    master_gvkey_list = master_gvkey.iloc[:, 0].values
