...
setting_params = {
    ...
    "kirin_config": {
        "cache_dir": Path(DATA_DIR).as_posix(),
...
factorfactory_dict = {
    ...
    # "cache_dir": "/raid/sr-storage/ff_factors",  # DGX-v2
    "cache_dir": "/raid/public/sr-storage/ff_factors_mixweight",  # DGX-v1
}
...
def run_model(context_name_list, context_list, dict_of_shapes, inference_date):
    di = DeepIntegration(
...
if __name__ == "__main__":
    ...
    for raw_data in raw_inputs:
        ...
        if raw_data.get_tag() == QraftDataTag.EQUITY.value:
            ...
            raw_data = raw_data.slice(columns=ff_columns)
    ...
    for d_inference in dates_inference:
        ...
        print(d_inference, "is running by youngmin")
        ...
        ## 1. single
        if not parallel:
            run_model(name_list, lc_list, shapes_dict, d_inference)

        ## 2. parallel
        # proc = mp.spawn(
        #     run_model,
        #     args=(name_list, lc_list, shapes_dict, d_inference),
        #     join=False,
        # ).processes[0]
        ...
        ## 3. parallel
            ...
            proc = mp.Process(
                target=run_model, args=(name_list, lc_list, shapes_dict, d_inference)
            )
            proc.start()
