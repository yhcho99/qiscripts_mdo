...
EPOCHS = 23
BATCH_SIZE = 32 # 256개 기업이 32개씩 들어간다
CROSS_NUM_ITER = 100
CROSS_NUM_UNIVERSE = 256
TRAINING_LENGTH = 48 # 샘플을 뽑는 기간?
SAMPLE_LENGTH = 36
...
def run_model(_, context_name_list, context_list, dict_of_shapes, inference_date):
    ...
    # 초기화 하면, model을 만들고, common variable을 설정함
    training_lc = context_list[0]
    inference_lc_list = context_list
    # context_list에는 local context 객체들의 리스트
    ...
    for epoch in range(1, integrated_model_params["epochs"] + 1):
        if early_stopping_flag: # early stopping flag가 뜨면 멈춤!
            break
...
#######
    ...
    y = 2 * y.slice(columns=ff_columns).rank(pct=True) - 1 # -1에서 1사이로 보내줌
