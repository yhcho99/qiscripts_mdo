...
#ㄷㅏ음실험시 맵퍼에서 all빼고 다 지우고 num_factors를 2로 바꿔준후 실행
universe_mapper = {
...
def run_model(_, context_name_list, context_list, dict_of_shapes, inference_date):
    ...
    # split_item은 1:training 이거나 2: validation인 tag임
    ...
    split_item: bm.splitting.TrainingValidationSplit = training_lc.training_split(
        # training_validation_split하는 function과 keyword들을 제공해주면 local_context객체의 self._training_split에 split된 결과가 저장되고, split_item에는 tag가 저장된다
...
if __name__ == "__main__":
    ...
    master_dataset_fp = Path(DATA_DIR) / "ff_master_dataset"
    ...
    alt_data = []
    for alt in alt_data_list:
        try:
            qdata = QraftData.load(alt, master_dataset_fp)
            print(f"load_data: {alt}")

        except FileNotFoundError:
            data_fp = Path(DATA_DIR) / "alt_master_csv" / f'{alt}.csv'
            data = pd.read_csv(data_fp,index_col=0, parse_dates=True)
            qdata = QraftData(alt, data)
            qdata.save(master_dataset_fp.as_posix())
            print(f'save data: {alt}')

        alt_data.append(qdata)

    alts = []
    for raw_data in alt_data:
        raw_data = 2 * raw_data.slice(index=input_index).rank(pct=True) - 1
        raw_data._data = raw_data._data.reindex(columns=ff_columns)
        alts.append(raw_data)
...
#######
    ...
    infer_filter_list=[]
    IPO_PERIODS = [12,24,36,48,60]
    for p in IPO_PERIODS:
        data_pf = Path(DATA_DIR) / 'IPO' / 'mask' / f'mask_{p}months.csv'
        data = pd.read_csv(data_pf, index_col=0, parse_dates=True)
        data = data.reindex(columns=infer_columns, index=infer_index).fillna(False)
        infer_filter = QraftData(f'mask_{p}months', data)
        infer_filter_list.append(infer_filter)

    name_list = [f'{U}months' for U in IPO_PERIODS]

########
    # Check input, output and filter dataset has same index and columns
    index, columns = bm.checking.check_equal_index_and_columns(
        inputs + alts + [y, infer_filter_base]
    )
#########
...
#          datetime.datetime(2006, 10, 31),
#          datetime.datetime(2007, 5, 31),
#          datetime.datetime(2008, 8, 31),
        
#          datetime.datetime(2012, 5, 31),
#          datetime.datetime(2013, 11,30),
#          datetime.datetime(2014, 3, 31),
#          datetime.datetime(2018, 10, 31),
