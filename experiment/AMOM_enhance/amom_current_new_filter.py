...
if __name__ == "__main__":
    ...
    filter_mv = filter_mv.rank(ascending=False, pct=True) <= 0.20

    pr_1m = load_dataset.call_if_not_loaded(
        "pr_1m_0m",
        ["high_level", "equity", "get_monthly_price_return"],
        [1, 0]
    ).masked_by(filter_mv).zscore()

    mom_3m = load_dataset.call_if_not_loaded(
        "mom_3m",
        ["high_level", "equity", "get_monthly_price_return"],
        [3, 0]
    ).masked_by(filter_mv).zscore()

    mom_6m = load_dataset.call_if_not_loaded(
        "mom_6m",
        ["high_level", "equity", "get_monthly_price_return"],
        [6, 0]
    ).masked_by(filter_mv).zscore()

    mom_12m = load_dataset.call_if_not_loaded(
        "mom_12m",
        ["high_level", "equity", "get_monthly_price_return"],
        [12, 0]
    ).masked_by(filter_mv).zscore()

    v = (mom_12m-pr_1m) + (mom_6m-pr_1m)*2 + (mom_3m-pr_1m)*4
    filter_infer = v.rank(ascending=False, pct=False) <= 100

    # Check input, output and filter dataset has same ㅂㄴㄴㅋ.;index and columns
