...
parser = argparse.ArgumentParser(description = 'amom volatility adjusted filter experiments')
parser.add_argument('--m12c', type = float, default = 1, help = '12 month momentum coefficient')
parser.add_argument('--m6c', type = float, default = 1, help = '6 month momentum coefficient')
parser.add_argument('--m3c', type = float, default = 1, help = '3 month momentum coefficient')
parser.add_argument('--test_num', type = int, help = 'test number')
...
IDENTIFIER = f'amom_enhance_test_vol_adj_m12x{m12_coef}_m6x{m6_coef}_m3x{m3_coef}+testNum{test_num}'
...
if __name__ == "__main__":
    ...
    price_data_without_masking = load_dataset.call_if_not_loaded(
        "price_data_without_masking", ["high_level", "equity", "get_monthly_price_data"],[],{'enable_masking':False}
    )

    vol_3m = load_dataset.call_if_not_loaded(
        "vol_3m", ["high_level", "equity", "get_monthly_volatility"], [3],
    )
    vol_6m = load_dataset.call_if_not_loaded(
        "vol_6m", ["high_level", "equity", "get_monthly_volatility"], [6],
    )
    vol_12m = load_dataset.call_if_not_loaded(
        "vol_12m", ["high_level", "equity", "get_monthly_volatility"], [12],
    )
    ...
    v12 = vol_12m.masked_by(filter_mv).zscore()
    v6 = vol_6m.masked_by(filter_mv).zscore()
    v3 = vol_3m.masked_by(filter_mv).zscore()

    vol_adj_mom12 = m12 / v12
    vol_adj_mom6 = m6 / v6
    vol_adj_mom3 = m3 / v3

    v = m12_coef * vol_adj_mom12 + m6_coef * vol_adj_mom6 + m3_coef * vol_adj_mom3
