...
strategy_dict = {
    ...
    "long_weighting_config": (
        weights.rank_sum_discounted_mix_with_top_market_weight(0.995, None, None, None),
    ),
...
if __name__ == "__main__":
    import sys
    print(sys.argv[1:])
    num_pick, num_port, active_factor = sys.argv[1:]

    setting_params["identifier"] = f"adai_pick{num_pick}_num{num_port}_factor{active_factor}"
    num_pick = int(num_pick)
    num_port = int(num_port)
    active_factor = float(active_factor) * 0.01
    
    strategy_dict["long_weighting_config"][0]["active_num"] = num_pick
    strategy_dict["long_weighting_config"][0]["final_port_num"] = num_port
    strategy_dict["long_weighting_config"][0]["weight"] = active_factor
