...
#################################### 공통설정부분 ##########################################
setting_params = {
    "identifier": "xx_qrft_macro",  # 실험의 이름입니다
    "kirin_config": {
...
if __name__ == "__main__":
    exp_num, stock_num, step = sys.argv[2], sys.argv[3], sys.argv[4]
    assert step in ("exit", "train", "simul")
    ...
    setting_params["identifier"] = setting_params["identifier"] + f"_exp{exp_num}_stock{stock_num}"
    strategy_dict["long_picking_config"]["up_bd"] = stock_num
    strategy_dict["long_weighting_config"][0]["active_num"] = stock_num
