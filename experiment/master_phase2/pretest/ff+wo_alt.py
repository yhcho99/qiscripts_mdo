...
if __name__ == "__main__":
    ...
    ## 아래 셋 중 하나는 무조건 실행되어야 함.
    # ffmanager.prepare_is_checked()  # For experiment, (팩팩 관련) 모든 데이터가 이미 있는 경우.
    ffmanager.prepare_experiment()  # For experiment, (팩팩 관련) 모든 데이터를 다운로드하고 시작.
    # ffmanager.prepare_inference()   # For inference, (팩팩 관련) 일부 데이터만 다운로드하고 시작. (나머지는 pipeline 방식으로 다운로드 될 것을 기대함.)
